    ...
    if artistLabel == 'connection':
        if keyX:
            ...
            for artist in artistsList_Dict[objectId]:
                artist.remove()
                if artist in point1List: point1List.remove(artist)
                if artist in point2List: point2List.remove(artist)
        ...
        if keyShift or keyCtrl:
            if event.artist == curve1:
                if point1 == None:
                    point1, = axs[0].plot(coordPoint[0], coordPoint[1], marker='.', color='b')
                    vline1 = axs[0].axvline(coordPoint[0], color='b', linestyle='--', linewidth=1, picker=True, pickradius=0, label='vline')
                    plt.draw()
            elif event.artist == curve2:
                if point2 == None:
                    point2, = axs[1].plot(coordPoint[0], coordPoint[1], marker='.', color='b')
                    vline2 = axs[1].axvline(coordPoint[0], color='b', linestyle='--', linewidth=1, picker=True, pickradius=0, label='vline')
                    plt.draw()
